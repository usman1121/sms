...
function MagneticFillButton({
    ...
    return (
        <motion.button
            ...
            className={cn(
                "relative overflow-hidden px-8 py-3.5 rounded-full font-bold text-sm shadow-lg shadow-blue-500/25",
                "outline-none focus-visible:ring-2 focus-visible:ring-[#60a5fa]/50",
...
export default function ProjectsPage() {
    ...
    const [visibleCount, setVisibleCount] = useState(() => {
        if (typeof window !== 'undefined') {
            ...
            if (saved) return Math.max(30, parseInt(saved, 10));
        }
        return 30;
    });
    ...
    const products = useMemo(() => {
        ...
        return portfolioData.projects.slice(0, 10).map((p, i) => ({
            title: p.title,
            link: `/projects/${p.slug}`,
            thumbnail: p.image || screenshotImages[i % screenshotImages.length]
        }));
    }, []);
    ...
    const categories = [
        { id: 'All', label: 'All 24 Modules', icon: Globe },
        { id: 'Academic & Pedagogy', label: 'Academic & Pedagogy', icon: Brain },
        { id: 'AI & Automation', label: 'AI & Autopilot', icon: Zap },
        { id: 'Administration', label: 'Administration', icon: LayoutGrid },
        { id: 'Finance & Operations', label: 'Finance & Store', icon: Database },
        { id: 'Portals & Community', label: 'Portals & Telegram', icon: Blocks },
        { id: 'Compliance & Infrastructure', label: 'Compliance & Tech', icon: Cpu },
    ];
                ...
                <div id="project-archive" className="max-w-[1536px] mx-auto relative z-10 pb-12 sm:pb-16 md:pb-20 px-4 sm:px-6 md:px-8">
                            ...
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-2">
                                ...
                                <div className="relative group w-full md:w-80">
                                    ...
                                    <div className="relative flex items-center bg-transparent rounded-xl hover:bg-white/5 overflow-hidden transition-colors">
                                        ...
                                        <input
                                            type="text"
                                            placeholder="Search modules, features, tools..."
                                            value={searchQuery}
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={cn(
                                                        ...
                                                        isActive
                                                            ? "bg-[#60a5fa]/15 text-[#60a5fa] border border-[#60a5fa]/30 font-semibold"
                                                            : "bg-transparent text-muted-foreground hover:text-foreground hover:bg-[#60a5fa]/5 border border-transparent"
                                                    )}
                                                >
                                                    <Icon className={cn("w-3.5 h-3.5", isActive ? "text-[#60a5fa]" : "text-muted-foreground group-hover:text-foreground")} />
                                                    <span>{cat.label}</span>
                                                </button>
                                            );
                                        })}
                                        ...
                                        {filters.map((f) => (
                                            <button
                                                ...
                                                className={cn(
                                                    ...
                                                    filter === f.key
                                                        ? 'bg-[#60a5fa] text-white shadow-sm font-bold shadow-blue-500/25'
                                                        : 'text-muted-foreground hover:text-foreground hover:bg-[#60a5fa]/10'
                                                )}
                                            >
                                                {f.label}
                                            </button>
                    ...
                    {/* View All Button — Magnetic Fill-Invert */}
                    {
                        filteredProjects.length > 24 && (
                            ...
                            >
                                <MagneticFillButton
                                    onClick={() => setVisibleCount(visibleCount < filteredProjects.length ? filteredProjects.length : 24)}
                                    showArrowFlip={visibleCount >= filteredProjects.length}
                                >
                                    {visibleCount < filteredProjects.length ? 'View All 24 Modules' : 'Show Less'}
                                </MagneticFillButton>
                            </motion.div>
                        )
                    }

                    {
                        filteredProjects.length === 0 && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
                                ...
                                <p className="text-lg text-white/50">No modules found matching your search</p>
                            </motion.div>
                        )
                    }
                    {/* Contact Section */}
