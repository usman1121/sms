...
import { ChevronDown, Trophy, Navigation, Briefcase, Rocket, BookOpen, ImageIcon, FileText, MessageCircle, CreditCard } from 'lucide-react';
    ...
    return (
        <span className="inline-flex ml-2 -translate-y-px align-middle">
            <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 bg-[#60a5fa]"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-[#60a5fa] shadow-[0_0_5px_rgba(96,165,250,0.8)]"></span>
            </span>
        </span>
    );
}
...
function MegaBoxBig({ href, icon: Icon, title, desc, theme, pathname }: any) {
    ...
    return (
        <Link href={href} className={cn(
            ...
            theme === 'dark'
                ? cn("bg-[#161616] hover:bg-[#1f1f1f] hover:shadow-xl hover:shadow-black/50", isActive ? "border-[#60a5fa]/50 shadow-[0_0_15px_rgba(96,165,250,0.1)]" : "border-white/10 hover:border-white/20")
                : cn("hover:bg-white hover:shadow-xl hover:shadow-black/10", isActive ? "bg-white border-[#60a5fa]/80 shadow-md shadow-[#60a5fa]/10" : "bg-black/[0.02] border-black/10 hover:border-black/20")
        )}>
            ...
            <Icon className={cn("w-6 h-6 relative z-10 transition-transform duration-500 group-hover:scale-110 group-hover:-rotate-3", theme === 'dark' ? (isActive ? "text-[#60a5fa]" : "text-white/70 group-hover:text-white") : (isActive ? "text-[#3b82f6]" : "text-black/70 group-hover:text-black"))} />

            <div className="relative z-10 mt-auto">
                <h4 className={cn("font-bold text-[15px] mb-1.5 transition-colors duration-300 flex items-center", theme === 'dark' ? (isActive ? "text-[#60a5fa]" : "text-white") : (isActive ? "text-[#3b82f6]" : "text-black"))}>
                    {title}
    ...
    return (
        <Link href={href} className={cn(
            ...
            <div className="flex items-start justify-between relative z-10">
                <div>
                    <h4 className={cn("font-bold text-sm mb-1.5 transition-colors duration-300 flex items-center", theme === 'dark' ? (isActive ? "text-[#60a5fa]" : "text-white") : (isActive ? "text-[#3b82f6]" : "text-black"))}>
                        {title}
                        {isActive && <ActiveDot theme={theme} />}
                    </h4>
                ...
                <div className={cn("p-1.5 rounded-xl transition-colors duration-300", theme === 'dark' ? "group-hover:bg-white/10" : "group-hover:bg-black/5")}>
                    <Icon className={cn("w-4 h-4 mt-0.5 flex-shrink-0 transition-transform duration-500 group-hover:scale-125 group-hover:rotate-12", theme === 'dark' ? (isActive ? "text-[#60a5fa]" : "text-white/40 group-hover:text-white/80") : (isActive ? "text-[#3b82f6]" : "text-black/40 group-hover:text-black/80"))} />
                </div>
            </div>
        </Link>
    )
...
function SidebarLink({ href, icon: Icon, title, desc, theme, pathname }: any) {
    ...
    const className = cn(
        ...
        theme === 'dark'
            ? cn("bg-[#161616] hover:bg-[#1f1f1f]", isActive ? "border-[#60a5fa]/50 shadow-[0_0_15px_rgba(96,165,250,0.1)]" : "border-white/10 hover:border-white/20")
            : cn("hover:bg-white", isActive ? "bg-white border-[#60a5fa]/80 shadow-sm shadow-[#60a5fa]/10" : "bg-black/[0.02] border-black/10 hover:border-black/20"),
    ...
    const content = (
        <>
            ...
            <Icon className={cn("w-5 h-5 relative z-10 transition-transform duration-500 group-hover:scale-125 group-hover:-rotate-6", theme === 'dark' ? (isActive ? "text-[#60a5fa]" : "text-white/40 group-hover:text-white/80") : (isActive ? "text-[#3b82f6]" : "text-black/40 group-hover:text-black/80"))} />
...
export default function CardNav({
    ...
    const allHrefs = ['/projects', '/blog', '/gallery', '/contact'];
                    ...
                    {isActive && isExpanded && (
                        ...
                        >
                            <motion.span
                                ...
                                className="w-1.5 h-1.5 rounded-full bg-[#60a5fa] shadow-[0_0_8px_rgba(96,165,250,0.8)]"
                            />
                        </motion.div>
                    )}
                    {aboutItem.label}
                        ...
                        <div className={cn(
                            ...
                            <div className="flex-1 p-5 flex flex-col gap-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <MegaBoxBig href="/projects" icon={Rocket} title="Core Modules" desc="24 unified educational & admin tools" theme={theme} pathname={pathname} />
                                    <MegaBoxBig href="/#pricing" icon={CreditCard} title="Institutional Plans" desc="Simple pricing scaled by student count" theme={theme} pathname={pathname} />
                                </div>
                                {/* Bottom 2 small boxes */}
                                <div className="grid grid-cols-2 gap-4">
                                    <MegaBoxSmall href="/blog" icon={BookOpen} title="Case Studies" desc="Grade 12 remediation & cashless tuition insights" theme={theme} pathname={pathname} />
                                    <MegaBoxSmall href="/gallery" icon={ImageIcon} title="Product Tour" desc="High-resolution dashboard screenshots" theme={theme} pathname={pathname} />
                                </div>
                            </div>
                            ...
                            <div className={cn(
                                ...
                                <SidebarLink href="/gallery" icon={ImageIcon} title="Product Tour" desc="Screenshots & interface previews" theme={theme} pathname={pathname} />
                                <SidebarLink href="https://www.yeneschooldocumentation.vercel.app" icon={FileText} title="Documentation" desc="Official platform & API guides" theme={theme} pathname={pathname} />
                                <SidebarLink href="https://t.me/yeneschool_bot" icon={MessageCircle} title="Telegram Bot" desc="Parent & emergency alert channel" theme={theme} pathname={pathname} />
                            </div>
                        </div>
                    </motion.div>
                )}
