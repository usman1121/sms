    ...
    <div className="mx-auto w-full max-w-3xl px-4 text-center sm:px-6">
      ...
      <div className="-m-1 flex flex-wrap justify-center gap-1 sm:gap-1.5">
        {testimonials.map((testimonial, index) => (
          <button
            key={index}
            className={`m-1 inline-flex justify-center rounded-full px-2 py-1 text-xs whitespace-nowrap shadow-sm transition-colors duration-150 focus-visible:ring focus-visible:ring-blue-300 focus-visible:outline-none sm:px-3 sm:py-1.5 sm:text-xs dark:focus-visible:ring-blue-600 ${active === index ? "bg-[#60a5fa] text-white shadow-blue-950/10 font-bold" : "bg-white dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:bg-blue-50 dark:hover:bg-zinc-700"}`}
            onClick={() => {
              setActive(index)
              setAutorotate(false)
            }}
          >
            ...
            <span
              className={`${active === index ? "text-blue-100" : "text-[#60a5fa]"}`}
            >
              -
            </span>{" "}
