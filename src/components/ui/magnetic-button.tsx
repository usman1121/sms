...
export function MagneticButton({ children, className, variant = 'primary', href }: MagneticButtonProps) {
    ...
    const primaryStyles = "bg-[#60a5fa] text-white border-2 border-[#60a5fa] shadow-lg shadow-blue-500/20";
    const outlineStyles = "bg-transparent text-neutral-900 dark:text-white border-2 border-neutral-300 dark:border-white/20 hover:border-[#60a5fa]";
    ...
    const content = (
        ...
        >
            ...
            <div className={cn(
                ...
                isHovered && variant === 'outline' ? 'text-white' : '',
                isHovered && variant === 'primary' ? 'text-white' : ''
            )}>
                {children}
            </div>
        </motion.div>
